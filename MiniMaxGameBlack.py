...
def MiniMaxGameBlack(depth,Board_Position,flag):
    ...
    if(depth==0):
        ...
        count_final = count_black - count_white
    ...
    for position in L:
        if(flag==1):
            param_inp = MiniMaxGameBlack(depth-1,position,0)
            ...
            param_inp = MiniMaxGameBlack(depth-1,position,1)
            if(param_inp.estimate<param_out.estimate):
                param_out.estimate=param_inp.estimate
                param_out.Board_Position = position
            param_out.nodes_crossed = param_out.nodes_crossed + param_inp.nodes_crossed
...
output = MiniMaxGameBlack(int(sys.argv[3]),input_string,1)
